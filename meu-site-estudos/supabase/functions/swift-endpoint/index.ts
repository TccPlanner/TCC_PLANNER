...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (authError || !user) return json(401, {
      ok: false,
      error: authError
    });
    ...
    if (action === "create_course") {
      const { data, error } = await supabaseAdmin.from("flash_courses").insert({
        user_id: user.id,
        nome
      }).select("id").single();
    ...
    if (action === "create_discipline") {
      ...
      if (!course_id) return json(400, {
        ok: false,
        error: "course_id é obrigatório."
      });
      const { data: course } = await supabaseAdmin.from("flash_courses").select("id").eq("id", course_id).eq("user_id", user.id).maybeSingle();
      if (!course) return json(403, {
        ok: false,
        error: "Curso inválido para este usuário."
      });
      const { data, error } = await supabaseAdmin.from("flash_disciplines").insert({
        user_id: user.id,
        course_id,
        nome
      }).select("id").single();
    ...
    if (action === "create_subject") {
      ...
      if (!discipline_id) return json(400, {
        ok: false,
        error: "discipline_id é obrigatório."
      });
      const { data: discipline } = await supabaseAdmin.from("flash_disciplines").select("id").eq("id", discipline_id).eq("user_id", user.id).maybeSingle();
      if (!discipline) return json(403, {
        ok: false,
        error: "Disciplina inválida para este usuário."
      });
      const { data, error } = await supabaseAdmin.from("flash_subjects").insert({
        user_id: user.id,
        discipline_id,
        nome
      }).select("id").single();
    ...
    if (action === "create_deck") {
      ...
      if (!subject_id) return json(400, {
        ok: false,
        error: "subject_id é obrigatório."
      });
      const { data: subject } = await supabaseAdmin.from("flash_subjects").select("id").eq("id", subject_id).eq("user_id", user.id).maybeSingle();
      if (!subject) return json(403, {
        ok: false,
        error: "Assunto inválido para este usuário."
      });
      const { data, error } = await supabaseAdmin.from("flash_decks").insert({
        user_id: user.id,
        subject_id,
        nome
      }).select("id").single();
    ...
    if (action === "create_card") {
      if (!deck_id) return json(400, {
        ok: false,
        error: "deck_id é obrigatório."
      });
      const { data: deck } = await supabaseAdmin.from("flash_decks").select("id").eq("id", deck_id).eq("user_id", user.id).maybeSingle();
      if (!deck) return json(403, {
        ok: false,
      ...
      if (payload.tipo === "normal" && (!payload.pergunta || !payload.resposta)) {
        return json(400, {
          ok: false,
          error: "pergunta/resposta são obrigatórias para tipo normal."
        });
      }
      if (payload.tipo === "cloze" && (!payload.cloze_text || !payload.cloze_answer)) {
        return json(400, {
          ok: false,
          error: "cloze_text/cloze_answer são obrigatórias para tipo cloze."
        });
      }
